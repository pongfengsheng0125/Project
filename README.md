...
2.在開發過程中，我最初以拋物線運動作為案例進行實作。但早期的程式僅支援單變數 ODE，例如衰減運動或簡單指數增長，無法處理二階或多變數系統。為了解決這個限制，我研究了如何將高階或多變數微分方程轉換為一階系統，並改進程式以支援系統 ODE，最終可以處理拋物線運動、簡諧運動等案例。
...
3.加入解函數支援，透過分段線性插值建立 y(x) 函數，不需使用者自行插值。
4.在Euler 與 RK4 方法中加入支持多變數、系統ODE的程式